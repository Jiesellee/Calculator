terraform {
  backend "s3" {
    ...
    key     = "order_prod/terraform.tfstate"
    region  = "eu-west-1"
    profile = "ri-dev"
  }
}
...
provider "aws" {
  ...
  allowed_account_ids = ["978203441460"]

  assume_role {
    role_arn = "arn:aws:iam::978203441460:role/OrganizationAccountAccessRole"
  }
}
...
data "aws_iam_policy_document" "developer_override_policy_document" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::order-data-extract-prod",
      "arn:aws:s3:::order-data-extract-prod/*",
    ]
  }
}
