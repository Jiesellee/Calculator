// This is the code which creates the management account for Styleman
// It was originally a copy (no longer in sync) of the code for order_prod
//
// Please note that this repo will only reference Stlyeman PROD,
// as other Styleman envs are temporary and were hand-rolled
//
terraform {
  backend "s3" {
    bucket  = "dev-core-tf-state"
    key     = "styleman_prod/terraform.tfstate"
    region  = "eu-west-1"
    profile = "ri-dev"
  }
}
...
provider "aws" {
  ...
  allowed_account_ids = ["154443489402"]

  assume_role {
    role_arn = "arn:aws:iam::154443489402:role/OrganizationAccountAccessRole"
  }
}
...
module "iam_member_account" {
  ...
  aws_product_management_account_id = "154443489402"
}
